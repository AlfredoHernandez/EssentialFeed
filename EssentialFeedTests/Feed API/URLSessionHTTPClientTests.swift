...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_performsGETRequestFromURL() {
        let url = anyURL()
        var exp: XCTestExpectation? = expectation(description: "Wait for request")
    ...
    func test_getFromURL_succedsWithEmptyDataOnHTTPURLResponseWithoutData() {
        let response = anyHTTPURLResponse
    ...
    private func resultValuesFor(data: Data?, response: URLResponse?, error: Error?,
        ...
        case let .success(result):
            return result
        default:
            XCTFail("Expected success, got failure \(result) instead.", file: file, line: line)
        }
        return nil
    }
    ...
    private var anyHTTPURLResponse: HTTPURLResponse { HTTPURLResponse(url: anyURL(), statusCode: 200, httpVersion: nil, headerFields: [:])! }
    private var anyNSError: NSError { NSError(domain: "any error", code: 1) }
